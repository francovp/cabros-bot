...
3. **Run the PR discussion loop after every PR creation or update**:
   - Take a baseline snapshot of paginated GraphQL `reviewThreads` (thread ID, creation time, author, resolved/outdated state) and paginated top-level PR conversation comments (comment ID, creation time, author, and body), then record the current head SHA. Flat comments alone are not sufficient for inline thread state, but top-level conversation comments must also be tracked.
   ...
   - Repeat the loop until a complete quiet window finishes with no new inline discussion or actionable top-level comment and no unresolved actionable thread remaining, or until the human-input exception routes the PR to Step 7. Compare thread IDs/timestamps and comment IDs/timestamps so a resolved item and a newly created item cannot cancel each other out.
