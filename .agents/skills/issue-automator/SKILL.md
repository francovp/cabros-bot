...
3. **Run the PR discussion loop after every PR creation or update**:
   ...
   - Wait using the quiet-window policy in `references/readiness-and-verification.md`, checking around the midpoint and at the end. Do not merge while this loop is active; hand off only through the explicit human-input exception below.
   - When a new or baseline discussion appears, triage and address every actionable unresolved thread before continuing. Use `github:gh-address-comments` for actionable review feedback; implement requested changes, reply when an explanation is sufficient, and resolve only when the discussion is actually handled.
   ...
   - Repeat the loop until a complete quiet window finishes with no new discussion and no unresolved actionable thread remaining, or until the human-input exception routes the PR to Step 7. A new thread count alone is insufficient: compare thread IDs/timestamps so a resolved thread and a newly created thread cannot cancel each other out.
