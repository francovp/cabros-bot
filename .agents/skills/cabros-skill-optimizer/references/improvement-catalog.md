...
| Railway | Before any env-var mutation or deploy, select the explicit project, environment, and service identifiers (`railway link --project <id> --environment production --service <id>` or equivalent CLI flags). Inspect live readiness via `https://cabros-bot-production.up.railway.app/healthcheck`. After a confirmed green deployment, use `pnpm run sync:production-env --key <NAME> --status railway=VERIFIED` to record the verified update. | Keep code/preview work; report production propagation unverified (`INTEGRATION_BLOCKED`). |
| Render | For **post-merge env-sync** (per AGENTS.md §Post-merge production environment synchronization), Render is still a required sync target alongside Railway and Vercel — use the `sync-production-env.js` helper to generate safe per-platform commands. For **skill-optimizer/CI skill work**, the `render` CLI is unavailable — classify that as `INTEGRATION_BLOCKED` and continue; do not let a missing `render` CLI block source analysis, tests, or skill maintenance. | For env-sync: record `render=VERIFIED` after confirmed sync. For skill work: report `render CLI unavailable` and continue. |
