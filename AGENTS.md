...
- **Audit drift**: `pnpm run sync:production-env --check-drift` diffs `.env.example` against platform blueprint definitions.
  - **Strict mode**: `pnpm run sync:production-env --check-drift --exit-on-drift` (alias `--strict`) returns exit code `2` when any `.env.example` key is not declared in `render.yaml`. The default behavior of `--check-drift` remains exit `0` for backward compatibility.
  - **CI enforcement**: `.github/workflows/env-drift-check.yml` runs `--check-drift --exit-on-drift` on a weekly schedule (Monday 09:00 UTC), on every push to `master` that touches `.env.example`/`render.yaml`/the helper script/the workflow file, and on demand via `workflow_dispatch`. A red job signals an unmanaged key that needs a `render.yaml` declaration or a verified sync record.
  - **Exit codes**: `0` for clean drift, plan generation, or audit record; `1` for CLI validation errors; `2` for drift detected under `--exit-on-drift`/`--strict`.
