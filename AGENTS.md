...
## HTTP Conditional GET on Read Endpoints (CB-198 / Issue #862)

Read-only `/api/*` endpoints now emit a deterministic SHA-1 weak ETag and `Cache-Control: private, max-age=0, must-revalidate` header so admin polling and operator dashboards can short-circuit unchanged reads to `304 Not Modified` with an empty body. The middleware is opt-in via `ENABLE_HTTP_CONDITIONAL_GET` (default `true`); Remote Config allow-lists the same key for runtime tuning. The `X-Conditional-Get` response header reports the per-request outcome (`hit`, `miss`, or `pass-through` for handler-set ETags such as scanner-presets version-based optimistic concurrency).

**Behavior**:
- The middleware computes a SHA-1 base64 ETag of the JSON response body and emits it under `W/"..."`; it short-circuits to 304 when the supplied `If-None-Match` header matches (including the unprefixed hash, the weak form, or `*`).
- Express's default weak-validator ETag generation is disabled on the owning app so the middleware owns conditional GET semantics; the gate flag suppresses the header entirely when `ENABLE_HTTP_CONDITIONAL_GET=false`.
- A handler that already set its own ETag (e.g. `scanner-presets` returning a version-based `ETag` for `If-Match` optimistic concurrency) is preserved verbatim, and the middleware only adds the `Cache-Control` header with an `X-Conditional-Get: pass-through` marker.
- `/api/status` and `/api/capabilities` expose `featureFlags.httpConditionalGet` and `dependencies.httpCache` (with `enabled`, `etagHits`, `etagMisses`, `bodyBytesSaved`, `shortCircuitedResponses`, `shortCircuitErrors`, and `lastResetAt`).
- Bounded to 100 KB JSON bodies; larger responses fall back to a 200 with no ETag and `etagMisses` is incremented.

**Core Components**:
- `src/lib/conditionalGet.js` — `withConditionalGet(handler)` factory, `etagFor({ updatedAt, count, version })` helper, `parseIfNoneMatch`/`ifNoneMatchMatches` validators, in-memory counter snapshot.
- `src/routes/index.js` — Wires `withConditionalGet` around every read endpoint and overrides Express's default ETag on the API router.
- `src/controllers/status.js` — Exposes `featureFlags.httpConditionalGet` and `dependencies.httpCache` (resolves `ENABLE_HTTP_CONDITIONAL_GET` from Remote Config runtime when available).
- `src/services/remoteConfig/RemoteConfigService.js` and `firebase-remote-config-template.json` — Remote Config allow-list entry for `ENABLE_HTTP_CONDITIONAL_GET`.
- `tests/unit/conditional-get.test.js`, `tests/integration/status-endpoint.test.js` — Cover matching 304, non-matching 200, missing header 200, weak-validator stripping, wildcard match, gate-disabled behavior, POST pass-through, handler-set ETag pass-through, and the dependency snapshot.
- `src/openapi/openapi.json` and `CabrosBot.postman_collection.json` — Document the `If-None-Match` parameter, 304 response, `HttpCacheDependency` schema, and example conditional GET request.
- `.env.example` — Documents `ENABLE_HTTP_CONDITIONAL_GET` with default and operational guidance.

Disabled by default preserves existing GET responses byte-for-byte (no ETag/Cache-Control additions from the middleware).
