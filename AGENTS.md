...
- GH-785 / CB-? (notification redrive idempotency): every redrive dispatch in `NotificationRedriveService.sweep()` now generates a deterministic `redrive:<deadLetterId>:<channel>:<attemptNumber>` key (`buildRedriveIdempotencyKey`) and threads it through `NotificationManager.sendToChannels` (alert payload + per-channel map + options). `NotificationManager` exposes the key on every `SendResult.idempotencyKey` and `parseNotificationRouting` accepts an optional caller-supplied `idempotencyKey` (≤256 chars) on `/api/webhook/alert` and `/api/webhook/message` for cross-channel dedupe parity with CB-127. The key is persisted as `lastIdempotencyKey` on `notificationDeadLetters` documents and in-memory records so retries are deterministic across processes. The `Redrive` redrive metadata block (`{ attemptNumber, idempotencyKey, deadLetterId }`) is attached to dispatched alerts for observability. README, OpenAPI, Postman, and unit coverage document the new contract; no endpoint, environment variable, or Remote Config contract changed.
