...
## Discord Per-Source Webhook Routing (GH-798)

Discord alerts now route by `alert.source` (with category / type / setupType / eventCategory / topic fallbacks) to dedicated Discord webhooks via a JSON config map, mirroring the existing Telegram topic routing pattern. Existing single-webhook deployments continue to work unchanged: a missing or unmatched source falls back to `DISCORD_WEBHOOK_URL`, and per-request `discordWebhookUrl` overrides always win.

**Configuration**:
- `ENABLE_DISCORD_SOURCE_ROUTING=false` — Opt-in feature flag (default `false`). Disabled state preserves the legacy single-webhook behavior byte-for-byte.
- `DISCORD_SOURCE_ROUTING_JSON` — JSON object mapping alert-source keys to Discord webhook URLs. Example:
  ```json
  {
  ...
  ```
  Both `ENABLE_DISCORD_SOURCE_ROUTING` and `DISCORD_SOURCE_ROUTING_JSON` are **environment-only** (Discord webhook URLs are secrets/destinations and are excluded from Firebase Remote Config per the standard parity policy).

**Resolution precedence** (per alert): per-request `discordWebhookUrl` override → exact source match → source alias-group match (`market-scanner`/`scanner`/`scanner-alert`/`scanner-alerts`/`market-scanner-alert`, `news-monitor`/`news`/`news-alert`/`news-monitor-alert`, etc.) → `default`/`*` fallback → `DISCORD_WEBHOOK_URL`.

**Core Components**:
- `src/services/notification/discordSourceRouting.js` — Parser, alias-group resolver, `isValidDiscordWebhookUrl` (re-uses the existing `requestRouting.js` URL pattern), and aggregate counters (decisions, fallbacks) plus `resetAggregateStatsForTesting`.
- `src/services/notification/DiscordService.js` — Adds `sourceRoutingEnabled`, `sourceRoutes`, `_resolveWebhookUrl()`, and `getSourceRoutingStatus()`. Per-request overrides bypass source routing entirely.
- `src/controllers/status.js` — Exposes `featureFlags.discordSourceRouting` and `dependencies.discordSourceRouting` (`enabled`, `configured`, `ready`, `status` = `ready`/`misconfigured`/`disabled`, `routesConfigured`, `hasDefault`, `keys`, `decisions`, `fallbacks`). Webhook URLs are never returned.

**Failure and Edge Case Behavior**:
- Disabled (`ENABLE_DISCORD_SOURCE_ROUTING != 'true'`) reports `status: disabled` and behaves byte-for-byte like the legacy single-webhook service.
- Empty / malformed / non-object JSON config fails open: warning logged and the default webhook is used.
- Unmatched source falls back to `DISCORD_WEBHOOK_URL` and increments the fallback counter.
- Per-request `discordWebhookUrl` always wins, even when source routing is enabled.

**Coverage**:
- `tests/unit/discord-source-routing.test.js` covers parsing, alias-group resolution, fallback precedence, aggregation counters, and URL validation.
- `tests/unit/discord-service.test.js` covers source-routed delivery, fallback to default, per-request override precedence, and the new `routeKey` field on `SendResult`.
- `tests/integration/status-endpoint.test.js` covers the protected `/api/status` `discordSourceRouting` contract under disabled, misconfigured, and ready states.
- `.env.example`, `src/openapi/openapi.json`, `CabrosBot.postman_collection.json` document the env vars, schema, and request/response examples.

No new Remote Config key was added; both env vars are environment-only per the standard secrets/destinations exclusion.
