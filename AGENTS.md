...
- GH-824: added opt-in `POST /api/ops/test-alert` post-deploy channel-delivery canary gated by `ENABLE_CANARY_ENDPOINT` (default `false`). Sends one synthetic alert (`source: 'canary'`) through the same `notificationManager.sendToChannels()` / `sendToAll()` pipeline as a real webhook alert — same formatter, retry, and redrive — with no Gemini/TradingView/Langfuse enrichment, so a deploy hook or GitHub Action can prove the just-deployed build can actually deliver. Returns per-channel `success` / `attemptCount` / `durationMs` / `errorCode` plus `successCount`/`failureCount` and `requestId` within the same `200` contract as the generic-message webhook. `400` for unknown/empty channels, `401`/`403` for `x-api-key` failures, `404 FEATURE_DISABLED` when the flag is off, `503` when notification services fail to initialize. Disabled by default to avoid accidental production spam; no new secret, no new Remote Config key, no public API or notification contract change beyond the additive endpoint. Coverage lives in `tests/integration/ops-canary-endpoint.test.js`, `tests/unit/ops-canary.test.js`, and the new status-endpoint canary-flag assertion.
