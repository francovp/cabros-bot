...
- **Audit jobs-worker mirror**: `pnpm run sync:production-env --check-worker-mirror --exit-on-worker-mirror-drift` confirms the dedicated `cabros-crypto-bot-telegram-worker` service declares every job-execution env var the web service exposes (TradingView MCP defaults, Remote Config, Notification redrive, zero-channel cooldown, API-only mode). The check compares the `envVars:` blocks in `render.yaml` and fails CI on a missing mirror.
