...
The system provides a zero-risk preview surface gated on `ENABLE_DEMO_MODE=true` (default `false`). Demo output is wrapped in a `_🧪 DEMO · synthetic data, do not trade_` banner, tagged `source: "demo"`, and is excluded from `shadowModeMetrics`, `signalOutcomeService.recordSignal`, and `alertFeedback` writes. The `docs/FEATURE_MATRIX.md` document is generated by `scripts/generate-feature-matrix.js` and validated against the actual codebase (env var references in `src/`, `featureFlags.*` and `dependencies.*` keys in `src/controllers/status.js`, and `/api/*` paths in `src/openapi/openapi.json`) on every regeneration.

**REST endpoints** (mounted in `src/routes/index.js` behind `validateApiKey`):
- `GET /api/demo/alert?text=...&channels=telegram,whatsapp,discord` — synthesizes an alert with mock enrichment and runs the per-channel dispatch path through `notificationManager.sendToAll`, tagged `source: "demo"` and never persisted to the Firestore `alerts` collection.
- `GET /api/demo/outcomes?symbol=BINANCE:BTCUSDT` — returns a fixed four-window cohort (`+1h`, `+4h`, `+1D`, `+1W`) with a deterministic `synthetic: true` marker; never queries any historical bar provider.
- `GET /api/demo/scanner?exchange=BINANCE` — returns a hardcoded three-item scanner list with a synthetic note; never calls TradingView MCP.

**Telegram command** (wired in `index.js` via `bot.command(['demo'], demoCmd)`):
- `/demo alert [text=...]` — replies with a MarkdownV2 preview wrapped in the DEMO banner.
- `/demo outcomes [symbol=...]` — replies with a fixed synthetic cohort summary.
- `/demo scanner [exchange=...]` — replies with a fixed synthetic scanner list.
- Subcommand must be `alert`, `outcomes`, or `scanner`; otherwise the bot replies with an inline error.
- When `ENABLE_DEMO_MODE` is unset the bot replies that demo mode is disabled.

**Configuration**:
- `ENABLE_DEMO_MODE` — Boolean flag (default `false`). Accepts `true`/`1`. Disabled in production until operators are ready to onboard new users.
- No new secrets or credentials are introduced; demo output is computed in-process.

**Core Components**:
- `src/controllers/demo.js` — exports `getDemoAlert`, `getDemoOutcomes`, `getDemoScanner`, `isDemoEnabled`, and `getDemoStatus`. Each handler is fail-open: external-service errors are caught and logged via `SentryService.captureRuntimeError`, returning `500 INTERNAL_ERROR`.
- `src/routes/index.js` — mounts the three demo routes behind `validateApiKey`. The demo status block is also appended to `dependencies.demo` in `src/controllers/status.js` and the `demoMode` flag is exposed under `featureFlags.demoMode`.
- `src/controllers/commands.js` — `demoCmd` parses subcommands and writes the MarkdownV2 reply; `index.js` registers `bot.command(['demo'], demoCmd)`.
- `scripts/generate-feature-matrix.js` — Generates `docs/FEATURE_MATRIX.md` from a curated `FEATURES` list, validates every entry against the live source (`process.env.*` references in `src/`, `featureFlags.*`/`dependencies.*` keys in `status.js`, and OpenAPI paths), and emits a drift report. Run `node scripts/generate-feature-matrix.js` to regenerate; pass `--check` to fail CI when the matrix is out of date.

**Where to look first when extending or debugging**:
- `src/controllers/demo.js` and `src/controllers/commands.js` for the preview payload contract.
- `src/routes/index.js` for endpoint registration.
- `src/controllers/status.js` for the `featureFlags.demoMode` and `dependencies.demo` exposure contract.
- Tests in `tests/unit/demo-controller.test.js`, `tests/integration/demo-endpoint.test.js`, and `tests/unit/generate-feature-matrix.test.js`.
...
## Demo Mode and Generated Feature Matrix (Issue #802)

Adds an opt-in `ENABLE_DEMO_MODE=false` flag plus `GET /api/demo/alert`, `GET /api/demo/outcomes`, `GET /api/demo/scanner` REST endpoints (mounted behind `validateApiKey`) and a Telegram `/demo` command with `alert`/`outcomes`/`scanner` subcommands. All demo output is wrapped in `meta: { synthetic: true }`, tagged `source: "demo"`, and is excluded from `signalOutcomeService.recordSignal`, `alertFeedback` collection writes, and the Firestore `alerts` persistence path. The status endpoint exposes `featureFlags.demoMode` and `dependencies.demo` (`enabled`, `configured`, `ready`, `status`) so operators can verify the gate without leaving `/api/status`. The feature is disabled by default and fails closed (404 `FEATURE_DISABLED`) when the env var is unset.

The new `scripts/generate-feature-matrix.js` produces `docs/FEATURE_MATRIX.md` from a curated `FEATURES` list, validates every entry against the live source (`process.env.*` references in `src/`, `featureFlags.*`/`dependencies.*` keys in `src/controllers/status.js`, and `/api/*` paths in `src/openapi/openapi.json`), and emits a drift report. Run `node scripts/generate-feature-matrix.js` to regenerate; pass `--check` for CI usage.

**Core Components**:
- `src/controllers/demo.js` — `getDemoAlert`, `getDemoOutcomes`, `getDemoScanner`, `isDemoEnabled`, and `getDemoStatus`. Each handler is fail-open: external-service errors are caught and reported via `SentryService.captureRuntimeError`.
- `src/controllers/commands.js` + `index.js` — `demoCmd` parses subcommands and writes the MarkdownV2 reply; `bot.command(['demo'], demoCmd)` wires the Telegram surface.
- `src/routes/index.js` — Mounts the three demo routes behind `validateApiKey`.
- `src/controllers/status.js` — Adds `featureFlags.demoMode` and `dependencies.demo` to the status payload.
- `scripts/validate-env.js` — Warns when `ENABLE_DEMO_MODE=true` is configured without an `ENABLE_TELEGRAM_BOT` or `WEBHOOK_API_KEY` gate (the demo routes would be unreachable).
- `scripts/generate-feature-matrix.js` — Generates `docs/FEATURE_MATRIX.md` with a drift report.
- `.env.example` — Documents `ENABLE_DEMO_MODE=false` with usage notes.
- `tests/unit/demo-controller.test.js`, `tests/integration/demo-endpoint.test.js`, `tests/unit/generate-feature-matrix.test.js` — Unit and integration coverage; the matrix test asserts zero drift against the live repository.

**Configuration**:
- `ENABLE_DEMO_MODE` — Opt-in flag (default `false`). Accepts `true`/`1`. Classified as **environment-only** for Remote Config parity (gated preview surface; existing `non-secret runtime tuning` rules do not apply — demo endpoints expose user-facing behavior, not operator tuning).

OpenAPI, Postman, and AGENTS.md updated with the new request/response contracts. No new secrets, credentials, or authentication flows were introduced.
