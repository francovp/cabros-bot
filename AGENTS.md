...
- `src/lib/requestDeadline.js` — Global API request-deadline middleware that returns `504 REQUEST_DEADLINE_EXCEEDED` with `X-Request-Id` and a Sentry `feature: api-request-deadline` event when an `/api/*` handler exceeds `API_REQUEST_DEADLINE_MS` (default `120000`, range `1000`-`600000`). Per-request overrides via `X-Cabros-Request-Deadline-Ms` are clamped to the same bounds; `/healthcheck`, `/ready`, `/openapi.json`, and `/docs` are exempt and the list can be extended with `API_REQUEST_DEADLINE_EXEMPT_PATHS`. The middleware is mounted in `app.js` between the rate limiter and the `/api` router, exposes a live counter via `dependencies.requestDeadline` in `/api/status`, and never blocks the response when Sentry or timers misfire (fail-open).
