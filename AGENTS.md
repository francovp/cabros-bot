...
- Scanner Preset Scheduler (`ScannerPresetSchedulerService.startWorker()`) runs recurring sweeps for due scanner presets with distributed lease locking when `ENABLE_SCANNER_PRESET_SCHEDULER=true`. `SCANNER_PRESET_SCHEDULER_WORKER_ROLE=web` (default) runs in web, `worker` runs in worker mode, `disabled` disables execution. A fixed per-preset 60s `MIN_PRESET_FLOOR_MS` minimum interval is enforced at runtime: presets whose `lastRunAt` is younger than the floor (or whose `nextRunAt` has drifted into the past after a cadence downgrade) are deferred, counted under `dependencies.scannerPresetScheduler.lastRunDeferredByFloorCount`, and skipped by both local web and dedicated-worker sweepers so MCP/Gemini/delivery burst traffic cannot be triggered by operator cadence tweaks. `/api/status` exposes `featureFlags.scannerPresetScheduler` and `dependencies.scannerPresetScheduler`.
