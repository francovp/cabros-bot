...
- Signal outcome windows are configured by `SIGNAL_OUTCOME_WINDOWS` (default `1h,4h,1D,1W`; allowed `1m,5m,15m,30m,1h,4h,1D,1W,1M`) with optional JSON display labels in `SIGNAL_OUTCOME_WINDOW_LABELS`. Invalid values fail open to the default, and each record persists its configured window keys and labels so historical data remains self-describing. Summary metrics expose only the currently configured windows; legacy records remain readable.
