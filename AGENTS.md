...
## Telegram /start Deep-Link Onboarding (chat enrollments)

The system exposes opt-in Telegram deep-link onboarding through the existing `/start` handler. Operators publish a single `t.me/<bot>?start=<token>` link in any rules message or channel post, and the bot resolves the payload to register the chat, set its language preference, pre-populate a watchlist, and capture a referral source.

**Core Components**:
- `src/controllers/commands/handlers/start/startPayload.js` — pure parser for the bounded `startPayload` string (max 64 chars) recognizing `enroll`, `lang=es|en`, `watch=SYM1,SYM2` (deduped, uppercased, regex-validated, capped at 20 symbols), and `ref=<token>` (max 64 chars). Unknown tokens are dropped silently; malformed tokens mark the payload `invalid: true` so the welcome flow falls back to the generic copy.
- `src/controllers/commands/handlers/start/startCommand.js` — composes the parser with the `ChatEnrollmentsService` and replies with a MarkdownV2 localized welcome plus an inline keyboard (`Add to group` URL, `/precio` and `/outcomes` callback buttons). The bot always replies, even when storage is disabled or enrollment fails — the persistence step is wrapped in try/catch and reports failures through Sentry.
- `src/services/enrollments/ChatEnrollmentsService.js` — opt-in Firestore collection `chatEnrollments` with a 90-day TTL (`CHAT_ENROLLMENT_RETENTION_DAYS` overrides), fail-open memory fallback when Firestore is unavailable, and `getStorageStatus()` for the `/api/status` surface.
- `src/controllers/webhooks/handlers/enrollments/enrollments.js` — `GET /api/enrollments` admin endpoint that returns the sanitized `count`, `languages`, and `watchlist` distributions, optionally augmented with sanitized per-chat records when the caller is an admin operator AND `includeChatIds=true`.
- `index.js` — registers `bot.start(startCmd)` (Telegraf's deep-link-aware hook) alongside the existing `bot.command(['help'], helpCmd)`. `bot.start()` sets `ctx.startPayload = ctx.payload`, so existing help-only `/help` invocations keep their current behavior while deep-link `/start foo` payloads route through the new handler.

**Behavior**:
- `/start` with no payload sends a localized welcome ("¡Hola! Soy Cabros Bot…") and never persists — the user must explicitly deep-link at least one recognized token to enroll.
- `/start enroll` and any payload containing `enroll`, `lang=…`, `watch=…`, or `ref=…` upserts the chat in the `chatEnrollments` collection (or memory fallback) and replies with the localized copy for the deepest recognized token (enrolled > language > watchlist > referral).
- Invalid payloads (`lang=zz`, `foo=bar`, payloads > 64 chars) are silently ignored and the user receives the generic welcome — the raw payload is never echoed back to the chat.
- Group enrollment via `t.me/<bot>?startgroup=enroll` is supported through Telegram's group-start deep link; the resolved `chatType` is `group` and the welcome copy is identical.

**Configuration**:
- `ENABLE_CHAT_ENROLLMENTS` — Opt-in feature flag (default `false`). When unset the parser still runs but `ChatEnrollmentsService` returns null on every operation, so the deep-link UX continues to work without storage.
- `CHAT_ENROLLMENT_RETENTION_DAYS` — Retention window for the Firestore TTL field on `chatEnrollments` documents (valid range: `1`-`365`, default `90`).
- Existing Firestore credentials (`FIREBASE_SERVICE_ACCOUNT_JSON` or `GOOGLE_APPLICATION_CREDENTIALS`) must be configured for the durable backend to come online; when they are missing the service stays in `ephemeral` mode and continues answering via memory.

**Admin surface**:
- `GET /api/enrollments` (admin viewer/operator) — sanitized aggregate `count`, `languages`, `watchlist`. Operators can request `?includeChatIds=true` to receive the per-chat records.
- `GET /api/status` and `GET /api/capabilities` expose `featureFlags.chatEnrollments` and `dependencies.chatEnrollments { enabled, configured, ready, status, mode, backend, count }`.
- OpenAPI: `GET /api/enrollments` documented under the `Chat enrollments` tag with the `EnrollmentsSummary` schema.
- Postman: a `Chat Enrollments` folder under the Cabros Bot collection with viewer + operator examples, `400 INVALID_REQUEST`, `401 UNAUTHORIZED`, `403 FEATURE_DISABLED`, and `503 STORAGE_UNAVAILABLE` response samples.
- `scripts/validate-env.js` (`pnpm run doctor`) warns when `ENABLE_CHAT_ENROLLMENTS=true` without a configured Firestore credential and validates `CHAT_ENROLLMENT_RETENTION_DAYS` against the bounded integer rules.

**Failure and Edge Case Behavior**:
- Persistence errors are fail-open: the welcome is still sent, the failure is logged via `console.warn` and reported to Sentry via the existing `sentryService.captureRuntimeError` hook.
- The memory fallback enforces an in-process Map keyed by chatId; reads check memory first, then Firestore (warm-on-miss), so a flapping backend cannot leak duplicate enrollments.
- The `/api/enrollments` endpoint surfaces `403 FEATURE_DISABLED` when the flag is off, `400 INVALID_REQUEST` for malformed `limit`, `401 UNAUTHORIZED` without an API key, and `503 STORAGE_UNAVAILABLE` when Firestore is enabled but unreachable.
- Chat IDs are never returned to admin viewers — only operators can request the per-chat records.

**Where to look first when extending or debugging**:
- `src/controllers/commands/handlers/start/startPayload.js` for parser rules and welcome copy.
- `src/controllers/commands/handlers/start/startCommand.js` for the handler composition and persistence wiring.
- `src/services/enrollments/ChatEnrollmentsService.js` for storage backend, fail-open semantics, and status snapshots.
- `src/controllers/webhooks/handlers/enrollments/enrollments.js` for the admin endpoint behavior.
- Tests in `tests/unit/start-payload.test.js`, `tests/unit/chat-enrollments-service.test.js`, and `tests/integration/start-command.test.js` for parser/service/handler coverage, plus `tests/integration/enrollments-endpoint.test.js` for the admin endpoint contract.
