...
- Worker Heartbeat Staleness Monitoring (`WorkerHeartbeatMonitorService`) periodically verifies background worker heartbeats across Firestore (`workerHeartbeats/signal-outcome`, `workerHeartbeats/scanner-preset-scheduler`) when their worker role is active. If a worker heartbeat exceeds `interval * WORKER_HEARTBEAT_STALE_MULTIPLIER` (default: 3x) or is missing after the startup grace period (`WORKER_HEARTBEAT_GRACE_PERIOD_MS`, default: 5m), it pages operators on `TELEGRAM_ADMIN_NOTIFICATIONS_CHAT_ID`. Paging alerts are deduplicated with a cooldown (`WORKER_HEARTBEAT_ALERT_COOLDOWN_MS`, default: 30m), and an all-clear recovery page is sent once heartbeats recover. `/api/status` exposes monitor readiness and state under `dependencies.workerHeartbeatMonitor`.
