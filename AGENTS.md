...
- **API Key Query-Parameter Deprecation (GH-756)**: The `api-key` query parameter is deprecated; credentials in URL query strings can leak through reverse-proxy access logs, browser history, and the repository's own middleware redaction. `validateApiKey` emits a one-time per-process `console.warn` on first use naming the route, and rejects query-parameter auth with `401 API_KEY_QUERY_REMOVED` once the optional `API_KEY_QUERY_SUNSET` (YYYY-MM-DD UTC) date is reached. The `x-api-key` header is the only supported path. Classified as environment-only for Remote Config parity (auth/transport sunset policy; excluded from the Remote Config template).
