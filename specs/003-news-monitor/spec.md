...
**Why this priority**: Current implementation strips URLs entirely from WhatsApp messages, losing valuable source attribution. URL shortening enables traders to verify alert sources while keeping messages readable and within transmission limits. The configured URL shortening service provides reliable link generation.

**Independent Test**: Can be fully tested by sending an alert with enriched citations to WhatsApp and verifying that each source URL is shortened (e.g., from 150+ chars to ~30 chars) and appears in the format "Title (short-url)" in the message. Verify fallback behavior when the configured provider is unavailable.

**Acceptance Scenarios**:

1. **Given** an alert is enriched with citations from Google GoogleSearch (which returns long redirect URLs like "https://vertexaisearch.cloud.google.com/grounding-api-redirect/..."), **When** formatting for WhatsApp, **Then** each URL is shortened to a user-friendly format (e.g., "https://short.url/abc123") via the configured URL shortening service
2. **Given** the configured URL shortening service succeeds, **When** the alert is sent to WhatsApp, **Then** the message includes formatted citations like "Reuters (https://short.url/abc1) / CoinDesk (https://short.url/def4)" instead of stripping URLs entirely
3. **Given** the configured URL shortening service call fails, times out, or is rate-limited, **When** shortening is attempted, **Then** the system gracefully preserves each citation's original URL and logs a warning at INFO level; alert delivery is NOT blocked
...
5. **Given** an in-memory cache of shortened URLs is maintained, **When** the same source URL is encountered in a second alert (within session lifetime), **Then** the cached short URL is reused without making another API call to the configured URL shortening service (reduces API quota consumption)
...
- **API timeouts (Binance, Gemini, Telegram, WhatsApp, URL shortener)**: Handled by `retryHelper.sendWithRetry()` with 3 retries and exponential backoff. If all retries fail, per-symbol result includes status "error" with timeout message. Notification channel failures do not block response (partial_success flag indicates mixed outcomes). URL-shortener failures do not block WhatsApp delivery (graceful fallback to original URLs).
...
- **URL shortening service rate limit**: After the configured URL shortening service rate limit is hit, system gracefully preserves original URLs for subsequent alerts and logs warning. Does not block alert delivery. Session-level cache prevents redundant API calls.
- **URL-shortener API returns invalid short URL format**: System validates response format and falls back to the original URL if an unexpected format is received. Logs error at WARN level.
...
- **FR-010**: System MUST optionally integrate with Binance API for crypto symbols when `ENABLE_BINANCE_PRICE_CHECK=true`. System does NOT validate symbol classification; requester ensures crypto symbols are placed in crypto array. If Binance API returns error (invalid symbol, rate-limit, network timeout), system returns per-symbol status "error" and continues analysis using Gemini-only sentiment without blocking alert delivery.
- **FR-011**: System MUST execute Binance and Gemini price fetches with aggressive and generous timeouts configurable via environment variables: `BINANCE_TIMEOUT_MS` (default: 5000 ms), `GEMINI_TIMEOUT_MS` (default: 20000 ms). If both calls fail or timeout, system returns analysis result without market price context (alert is still sent based on news sentiment). If one completes, use that result. Per-symbol timeout budget remains 30 seconds total.
- **FR-012**: System MUST apply a timeout of configurable duration (default: 30000 ms via `NEWS_TIMEOUT_MS`) to the entire analysis flow per symbol
...
- **FR-016**: System MUST format alerts for each notification channel using existing formatters: MarkdownV2 for Telegram (with inline markdown links), WhatsApp-compatible format for WhatsApp (with shortened URLs via a supported provider)
- **FR-017**: System MUST handle and gracefully log errors from external API calls (Gemini, Binance, Telegram, WhatsApp, optional secondary LLM, URL shortener) without stopping the entire monitoring flow. All external API calls MUST use `retryHelper.sendWithRetry()` with 3 retries and exponential backoff (see FR-018).
- **FR-018**: System MUST retry transient API failures using exponential backoff: Binance (3 retries, `BINANCE_TIMEOUT_MS` default 5000 ms), Gemini (3 retries, `GEMINI_TIMEOUT_MS` default 20000 ms), optional secondary LLM enrichment (3 retries, `LLM_TIMEOUT_MS` default 10000 ms), Telegram/WhatsApp (3 retries, `NOTIFICATION_TIMEOUT_MS` default 10000 ms), URL shortening (3 retries, `URL_SHORTENER_TIMEOUT_MS` default 5000 ms). Per-symbol 30s budget accounts for worst-case retry scenarios. Secondary LLM enrichment and URL shortening are independent and do not block alert delivery if they fail.
- **FR-019**: (Deferred to Feature 004: llm-alert-enrichment) System MUST support optional secondary LLM enrichment via `ENABLE_LLM_ALERT_ENRICHMENT` environment variable (default: false). When enabled, secondary LLM receives grounding results and returns refined confidence, reasoning, and recommended action. For 003 MVP, this feature is disabled and Gemini-only analysis is used.
- **FR-020**: (Deferred to Feature 004: llm-alert-enrichment) When enrichment is applied, system MUST apply conservative confidence selection: use minimum of (Gemini confidence, Secondary LLM confidence) to prevent false positives from LLM hallucination. For 003 MVP, Gemini-only analysis is used.
- **FR-022**: System MUST shorten source URLs when formatting alerts for WhatsApp using the configured URL shortening service. Configuration: `URL_SHORTENER_SERVICE` environment variable (default: `picsee`; options: `picsee`, `tinyurl`, `cuttly`). Service-specific tokens are `PICSEE_API_KEY` and `CUTTLY_API_KEY`; TinyURL requires no credential. Bitly, reurl, and Pixnet0rz.tw are unavailable. When enabled, all citations in enriched alerts are shortened before sending to WhatsApp. If the service call fails, times out, or rate-limit is hit, system gracefully preserves the original URLs without blocking WhatsApp delivery. Failures are logged at INFO level. Shortened URLs are cached in-memory (session-scoped) to reduce redundant API calls: cache key is original URL, value is `{ shortUrl, timestamp }`. Cache resets on bot restart. Acceptable for MVP; Phase 2 can add TTL-based eviction or Redis backing for distributed instances.
- **FR-024**: System MUST implement verbose structured logging for operational visibility: log each external API call (Gemini, Binance, Telegram, WhatsApp, URL shortening) with request/response summaries at DEBUG/INFO level. Include per-symbol analysis timing, cache hits, enrichment decisions, URL shortening results, and retry attempts. Log format should support easy parsing for Phase 2 Datadog integration. All errors, retries, and timeouts MUST be logged at WARN/ERROR level with correlation IDs (requestId) for audit trails. URL shortening errors should log at INFO level (graceful degradation, not fatal).
...
- **SC-016**: WhatsApp URL shortening (via configured service) reduces average message size from 25K characters (with long redirect URLs) to <10K characters (with shortened URLs), improving WhatsApp delivery reliability and reducing truncation
- **SC-017**: 90% of URL shortening requests succeed within 2 seconds (retried up to 3 times on failure). URLs are cached in-memory to reduce redundant API calls. Service failures gracefully preserve original URLs without blocking WhatsApp delivery.
- **SC-018**: URL shortening cache prevents redundant API calls: 80%+ of source URLs in sequential alerts are cache hits, reducing API quota consumption by ~75% in typical scenarios
- **SC-019**: Shortened URLs appear in WhatsApp messages in user-friendly format: "Title (https://short.url/abc123)" instead of omitting the source URL or exposing a long redirect URL

## Clarifications

### Session 2025-11-01 (URL Shortening Feature)

- **Q1: URL Shortening Library & API** → A: Use native fetch with the supported URL-shortening APIs (TinyURL, PicSee, Cutt.ly). Configuration uses `PICSEE_API_KEY` or `CUTTLY_API_KEY` where required; TinyURL requires no credential. If the service is unavailable or rate-limited, gracefully preserve original URLs without blocking delivery.
- **Q2: URL Extraction & Caching Strategy** → A: Do NOT attempt to extract destination domain from Google's redirect wrapper URLs. Use the full redirect URL as-is. Maintain in-memory session-scoped cache keyed by original URL, value is shortened URL. Cache resets on bot restart and prevents redundant provider calls for duplicate sources.
...
### Session 2025-11-08 (Generic URL Shortening Support)

- **Q1: URL Shortening Service Selection** → A: Support the runtime services: TinyURL, PicSee, and Cutt.ly. Bitly, reurl, and Pixnet0rz.tw are unavailable.
- **Q2: Service Configuration** → A: Configure via `URL_SHORTENER_SERVICE` (default: `picsee`). Use `PICSEE_API_KEY` or `CUTTLY_API_KEY` for the keyed services; TinyURL is enabled by setting the service name and requires no credential.
- **Q3: Fallback Strategy** → A: If the selected service is unavailable or fails, preserve original URLs. Log at INFO level. Do not block alert delivery.
- **Q4: Cache Strategy** → A: Maintain in-memory session-scoped cache for shortened URLs, keyed by original URL. Cache prevents redundant API calls for duplicate sources.
- **Q5: Timeout and Retry** → A: Use retryHelper.sendWithRetry() with 3 retries and exponential backoff (~5s timeout per call). Independent of alert delivery.
...
- **Retry logic reuses existing `retryHelper.sendWithRetry()`**: 3 retries with exponential backoff for Binance (~5s), Gemini (~20s), optional LLM enrichment (~10s), Telegram/WhatsApp (~10s), and URL shortening (~5s). Per-symbol 30s budget is sufficient for worst-case scenarios.
...
- **Configured URL shortening service is available** when the required token is configured; graceful fallback to original URLs if unavailable
