...
1. **Given** an alert is enriched with citations from Google GoogleSearch (which returns long redirect URLs like "https://vertexaisearch.cloud.google.com/grounding-api-redirect/..."), **When** formatting for WhatsApp, **Then** each URL is shortened to a user-friendly format (e.g., "https://bit.ly/abc123") via the configured URL shortening service
2. **Given** the configured URL shortening service succeeds, **When** the alert is sent to WhatsApp, **Then** the message includes formatted citations like "Reuters (https://bit.ly/abc1) / CoinDesk (https://bit.ly/def4)" instead of stripping URLs entirely
...
- **FR-022**: System MUST shorten source URLs when formatting alerts for WhatsApp using the configured URL shortening service. Configuration: `URL_SHORTENER_SERVICE` environment variable (default: 'bitly'; options: bitly, tinyurl, picsee, reurl, cuttly, pixnet0rz.tw). Service-specific tokens via `BITLY_ACCESS_TOKEN`, `TINYURL_API_KEY`, `PICSEE_ACCESS_TOKEN`, `REURL_ACCESS_TOKEN`, `CUTTLY_ACCESS_TOKEN` (required only for services that need them). When enabled, all citations in enriched alerts are shortened before sending to WhatsApp. If the service call fails, times out, or rate-limit is hit, system gracefully falls back to title-only citations without blocking WhatsApp delivery. Failures are logged at INFO level. Shortened URLs are cached in-memory (session-scoped) to reduce redundant API calls: cache key is original URL, value is `{ shortUrl, timestamp }`. Cache resets on bot restart. Acceptable for MVP; Phase 2 can add TTL-based eviction or Redis backing for distributed instances.
...
- **Q1: URL Shortening Service Selection** → A: Support multiple URL shortening services: Bitly, TinyURL, PicSee, reurl, Cutt.ly, Pixnet0rz.tw
- **Q2: Service Configuration** → A: Configure via URL_SHORTENER_SERVICE environment variable (default: 'bitly'). Provide service-specific tokens via env vars (BITLY_ACCESS_TOKEN, TINYURL_API_KEY, PICSEE_ACCESS_TOKEN, REURL_ACCESS_TOKEN, CUTTLY_ACCESS_TOKEN). Services without tokens (TinyURL, Pixnet0rz.tw) are enabled by setting the service name.
