...
3. **Given** the configured URL shortening service call fails, times out, or is rate-limited, **When** shortening is attempted, **Then** the system gracefully falls back to using the title only (current behavior: "Reuters / CoinDesk") and logs a warning at INFO level; alert delivery is NOT blocked
...
- **API timeouts (Binance, Gemini, Telegram, WhatsApp, URL shortener)**: Handled by `retryHelper.sendWithRetry()` with 3 retries and exponential backoff. If all retries fail, per-symbol result includes status "error" with timeout message. Notification channel failures do not block response (partial_success flag indicates mixed outcomes). URL-shortener failures do not block WhatsApp delivery (graceful fallback to title-only citations).
...
- **URL shortening service rate limit**: After the configured URL shortening service rate limit is hit, system gracefully falls back to title-only citations for subsequent alerts and logs warning. Does not block alert delivery. Session-level cache prevents redundant API calls.
- **URL-shortener API returns invalid short URL format**: System validates response format and falls back to title-only citation if unexpected format received. Logs error at WARN level.
...
- **FR-022**: System MUST shorten source URLs when formatting alerts for WhatsApp using the configured URL shortening service. Configuration: `URL_SHORTENER_SERVICE` environment variable (default: `picsee`; options: `picsee`, `tinyurl`, `cuttly`). Service-specific tokens are `PICSEE_API_KEY` and `CUTTLY_API_KEY`; TinyURL requires no credential. Bitly, reurl, and Pixnet0rz.tw are unavailable. When enabled, all citations in enriched alerts are shortened before sending to WhatsApp. If the service call fails, times out, or rate-limit is hit, system gracefully falls back to title-only citations without blocking WhatsApp delivery. Failures are logged at INFO level. Shortened URLs are cached in-memory (session-scoped) to reduce redundant API calls: cache key is original URL, value is `{ shortUrl, timestamp }`. Cache resets on bot restart. Acceptable for MVP; Phase 2 can add TTL-based eviction or Redis backing for distributed instances.
...
- **SC-017**: 90% of URL shortening requests succeed within 2 seconds (retried up to 3 times on failure). URLs are cached in-memory to reduce redundant API calls. Service failures gracefully fall back to title-only citations without blocking WhatsApp delivery.
...
- **SC-019**: Shortened URLs appear in WhatsApp messages in user-friendly format: "Title (https://short.url/abc123)" instead of current format (title only) or problematic format (long redirect URL)
...
- **Q3: Fallback Strategy** → A: If the selected service is unavailable or fails, fall back to title-only citations. Log at INFO level. Do not block alert delivery.
