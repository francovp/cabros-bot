...
def main():
    ...
    if overbought.empty:
        ...
        for _, row in overbought.iterrows():
            if not first:
                lines.append("")  # blank line between ticker blocks? Actually format shows each ticker on its own line with bullet, no blank lines needed? We'll add blank line for readability.
            first = False
            ticker = row['Name']
            price = row['Price']
            change = row['Change %']
            rsi = row['Rsi9']
            # Note: we can generate a brief caution based on RSI being high
