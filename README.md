...
The Binance Spot order workflow ships with an operator-gated **runtime kill-switch** so incident response can stop new submissions without waiting for an env change and a redeploy. The pause state is stored in a small Firestore document (`tradingControl/state`) and is **intentionally excluded from Firebase Remote Config** to stay consistent with the documented policy that route/security gates stay environment-only. The kill-switch is automatically available whenever `ENABLE_BINANCE_TRADING=true`; no new environment variable is required.

- `GET /api/trading/binance/pause` — read the effective pause state. Requires `admin.viewer` (or stronger).
- `POST /api/trading/binance/pause` — flip the kill-switch on. Requires `admin.operator`, accepts an optional `reason` string, and respects the existing idempotency middleware. Idempotent.
- `POST /api/trading/binance/resume` — clear the kill-switch. Requires `admin.operator`, accepts an optional `reason` string, and respects the existing idempotency middleware. Idempotent.

While the kill-switch is active, every `POST /api/trading/binance/orders` request — including `dryRun: true`, which still issues a signed `testNewOrder` call — fails with `503 TRADING_PAUSED` *before* any Binance client is constructed or any signed request is sent. The same response is returned when the authoritative pause state storage is unreachable while trading is enabled (fail-closed); when trading is disabled the gate is open by definition. The pause state is also surfaced in `/api/status` under `dependencies.binanceTrading` (`paused`, `pausedBy`, `pausedAt`, `pausedReason`, `lastChangedAt`, `lastChangedBy`, `lastAction`, `controlStorage`) and in `featureFlags.binanceTradingKillSwitch`. No API credentials, secrets, or provider responses are ever exposed through these surfaces.
