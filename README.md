...
- `WEBHOOK_API_KEY` - API key used to secure `/api/*` webhook endpoints. Required in production-like environments (`NODE_ENV=production`, Render, Vercel, Railway), where endpoints fail-closed with HTTP 503 if unset. When configured, clients must provide the key via the `x-api-key` header (or the `api-key` query parameter). This key is **scoped to `/api/webhook/*` and `/api/news-monitor`**; once `ADMIN_API_KEY` is set, the webhook key is no longer accepted on admin/operator routes
- `ADMIN_API_KEY` - Optional dedicated credential for the browser admin console and the `/api/admin/*` read/write surfaces (alerts, scanner-presets, jobs, outcomes, status, capabilities, replay). When set, the webhook key is no longer accepted on those routes. Firebase ID tokens remain the primary browser-admin path; the API key is the machine-client path. Excluded from Remote Config (environment-only secret)
- `BINANCE_TRADING_API_KEY` - Optional dedicated credential for Binance Spot order execution (`POST/GET/DELETE /api/trading/binance/orders`). When set, the webhook and admin keys are no longer accepted on the trading surface. When unset, admin authentication (Firebase or `ADMIN_API_KEY`) is the fallback. Excluded from Remote Config (environment-only secret)
...
`POST /api/trading/binance/orders` requires `admin.operator` access through the existing API-key or Firebase admin authentication flow, and fails closed if neither mechanism is configured. When `BINANCE_TRADING_API_KEY` is configured the trading surface is scoped to that key alone — neither the webhook key nor `ADMIN_API_KEY` is accepted — so a leaked webhook/admin key cannot execute trades. It supports `MARKET` and `LIMIT` `BUY`/`SELL` orders, validates the live Binance symbol status and filters, and uses the existing `binance` `MainClient`. MARKET orders accept either `quoteOrderQty` or base asset `quantity` (evaluated using average price against the configured notional cap). Quantity-based MARKET BUYs are converted to an exchange-enforced `quoteOrderQty` at the estimated average price so Binance itself caps the realized quote spend at `BINANCE_TRADING_MAX_NOTIONAL`; quantity-based MARKET SELLs keep base-quantity sizing.
...
2. Rotate `WEBHOOK_API_KEY`, `ADMIN_API_KEY`, and `BINANCE_TRADING_API_KEY` (each is independent — rotate the one that leaked, not the others) in the production secret store, then redeploy and verify protected endpoints with the new key. The three credentials are scoped per trust domain (webhook / admin / Binance trading) and can be rotated independently.
