...
- `API_REQUEST_DEADLINE_MS` - Global API request-deadline budget in milliseconds (default: `120000`; range `1000`-`600000`; values outside the range are clamped with a `console.warn`). Any `/api/*` handler that exceeds the budget returns `504 REQUEST_DEADLINE_EXCEEDED` with an `X-Request-Id` header and a Sentry `feature: api-request-deadline` event. Per-endpoint deadlines (`EXPANDED_ANALYSIS_ALERT_TIMEOUT_MS`, `MARKET_SCANNER_TIMEOUT_MS`, etc.) continue to coexist and win when they fire first; an existing per-endpoint timeout code is preserved as `innerCode` in the 504 body. Healthchecks, `/ready`, `/openapi.json`, and `/docs` are exempt. Per-request overrides via `X-Cabros-Request-Deadline-Ms` are clamped to the same bounds. `dependencies.requestDeadline` exposes `defaultMs`, `hardCapMs`, `minMs`, `activeTimers`, and `exemptPaths` through `/api/status` and `/api/capabilities`.
- `API_REQUEST_DEADLINE_EXEMPT_PATHS` - Comma-separated list of additional paths exempt from the global request deadline. Defaults to `/healthcheck`, `/ready`, `/openapi.json`, `/docs`; an extended list replaces the defaults when supplied. Operators can add a specific endpoint (e.g. `/api/jobs/:id`) without removing the defaults.
