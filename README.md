...
**Plan-quality gate (R:R floor):** When `SYMBOL_ANALYSIS_MIN_RISK_REWARD_RATIO` (default `1.5`, Firebase Remote Config eligible) is configured, plans with `risk.risk_reward_ratio` below the floor are downgraded to `decision.action: "NO_TRADE"` with `risk.valid: false`, `risk.rejectionReason: "risk_reward_below_minimum"`, and an explicit Spanish warning (`⚠️ R/R 0.17 por debajo del mínimo 1.5 — plan descartado.`) rendered in the alert text. Before flagging the plan, the controller upgrades the take-profit to the next S/R level, the opposite Bollinger band, or the ATR target — in that documented order — so a structurally-sound plan isn't lost just because the nearest S/R level sits too close to entry. Other rejection reasons (`missing_levels`, `invalid_directional_levels`) preserve the existing fail-open behavior.
