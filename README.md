...
- **CORS & CSP Policy**: Backend CORS reflects `Access-Control-Allow-Origin` only for the explicit origin allowlist (`CORS_ALLOWED_ORIGINS`, classified as environment-only; defaults: hosted console `https://cabros-bot.web.app` and `https://cabros-bot.firebaseapp.com`, ephemeral Firebase PR preview channels `https://cabros-bot--*.web.app`, plus the Railway backend `https://cabros-bot-production.up.railway.app`). Server-to-server and same-origin requests without an `Origin` header pass through untouched. Helmet CSP allows `connect-src` to Google Auth, Firebase Hosting origins, and the backend origin.
