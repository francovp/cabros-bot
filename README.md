...
`dependencies.signalOutcomeWorker` reports the scheduler role, shutdown state, cadence/budgets, and the last-sweep heartbeat counters (`lastRunAt`, scanned, pending, evaluated, error counts, plus `lastRunSkipped` / `lastRunSkipReason: 'lease_held'` and `lastLeaseRenewedAt` / `lastLeaseReleasedAt` for the cross-instance sweep lease). The `worker` role is intended for the dedicated Render service; set the web service role to `disabled` during cutover so only one scheduler is active. A disabled local scheduler reports `ready: false` and `status: "disabled"` because it is not the process evaluating outcomes.
...
The cross-instance sweep lease is acquired transactionally before each sweep on the `workerLeases/signal-outcome-sweep` document (`lockedUntil` / `lockedBy`); expired leases are takeover-eligible so a crashed holder never stalls evaluation. While Firestore is enabled the lease guarantees at-most-one evaluator across processes (zero-downtime deploys, future horizontal scale-out); while Firestore is disabled the in-memory single-flight guard preserves today's behavior. Configure `SIGNAL_OUTCOME_SWEEP_LEASE_MS` (default `60000`, bounded `10000`-`600000`) and `SIGNAL_OUTCOME_SWEEP_LEASE_RENEW_INTERVAL_MS` (default `20000`) per-instance in `.env.example`.
