...
`POST /api/trading/binance/orders/preview` returns a pre-trade cost preview without ever calling `submitNewOrder`. It is exposed behind the existing `admin.viewer`/`admin.operator` flow (API-key or Firebase bearer) and fails closed if neither mechanism is configured, mirroring the live endpoint. The response includes the LOT_SIZE-adjusted quantity, projected notional, the symbol's LOT_SIZE / MARKET_LOT_SIZE / PRICE_FILTER / NOTIONAL / MIN_NOTIONAL constraints from cached exchange-info, the maker/taker fee estimate derived from `account.commission` (or `10` bps by default) labeled `estimate — not a Binance fill guarantee`, the effective price source (`limitPrice` for LIMIT orders, `avgPrice` for MARKET orders, `quoteOrderQty` when only quote-asset quantity is supplied), and a 5-second `expiresAt` token so the preview cannot be replayed against a now-stale book. MARKET BUY orders additionally fetch `GET /api/v3/depth` with a 4-second `AbortController` deadline (fail-open `null` on transport failure) and expose a slippage estimate in basis points; an optional `maxSlippageBps` request field causes the response to report `wouldExceedBudget: true` without rejecting the preview itself. `BINANCE_TRADING_MAX_NOTIONAL` is enforced — previews above the cap return `403 MAX_NOTIONAL_EXCEEDED` even though no order is submitted. The preview never mutates Binance and is a no-op when `ENABLE_BINANCE_TRADING=false`.
