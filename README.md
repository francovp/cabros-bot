...
The initial allow-list contains news thresholds, timeouts, concurrency, quota retries, TradingView timeouts/retries, `SIGNAL_OUTCOME_RETENTION_DAYS` (retention in days between `1` and `3650`, default `365`), `ENABLE_MESSAGE_FOOTER_METADATA`, and the inline chart attachment controls (`ENABLE_CHART_ATTACHMENTS`, `CHART_RENDER_TIMEOUT_MS`, `CHART_CACHE_TTL_MS`, `CHART_CACHE_MAX_ENTRIES`, `CHART_DEFAULT_WIDTH`, `CHART_DEFAULT_HEIGHT`). Remote values are parsed as numbers/booleans and must satisfy the existing finite, integer, positive, and range constraints. Credentials, API keys, webhook authentication, route/security gates, and Telegram destinations are never read from Remote Config.
...
`featureFlags.chartAttachments` reports the `ENABLE_CHART_ATTACHMENTS` setting. When enabled and the alert envelope includes a `chartBuffer` (PNG bytes produced by `src/services/notification/charts/chartRenderer.js`), `TelegramService` uses `sendPhoto` with the formatted text as the caption (truncated to Telegram's 1024-character limit) and falls back to text-only delivery on any photo failure. WhatsApp and Discord currently fall back to text-only and log a `console.warn`. `dependencies.chartRenderer` exposes the redacted in-memory snapshot: `enabled`, `renderCount`, `cacheHitCount`, `cacheMissCount`, `successCount`, `failureCount`, `lastFailureCategory` (`INVALID_INPUT`/`TIMEOUT`/null), `lastRenderedAt`, `cacheSize`, `cacheMaxEntries`, `timeoutMs`, and `cacheTtlMs`. The renderer is process-local, dependency-free (no `canvas`/`sharp`), and fail-open: invalid inputs, timeouts, and render errors all return `null` so alert delivery proceeds unchanged. Chart buffers are never persisted, logged, or returned in `/api/status`.
