...
- `WORKER_STARTUP_JITTER_MS` - Global default upper bound (ms) for the per-process random delay applied before each background worker's first sweep or refresh (`0`-`30000`, default: `5000`). Set to `0` to disable jitter entirely. Workers: signal-outcome, notification-redrive, scanner-preset scheduler, Firebase Remote Config refresh, dedicated job worker.
- `SIGNAL_OUTCOME_WORKER_STARTUP_JITTER_MS` - Per-worker override (`0`-`30000`); falls back to `WORKER_STARTUP_JITTER_MS`.
- `NOTIFICATION_REDRIVE_WORKER_STARTUP_JITTER_MS` - Per-worker override (`0`-`30000`); falls back to `WORKER_STARTUP_JITTER_MS`.
- `SCANNER_PRESET_SCHEDULER_STARTUP_JITTER_MS` - Per-worker override (`0`-`30000`); falls back to `WORKER_STARTUP_JITTER_MS`.
- `REMOTE_CONFIG_REFRESH_JITTER_MS` - Per-worker override (`0`-`30000`) for the Firebase Remote Config refresh timer; falls back to `WORKER_STARTUP_JITTER_MS`.
...
`dependencies.startupJitter` reports the effective per-worker startup-jitter upper bound (ms) applied before each background worker's first sweep or refresh after a process start. Workers (`signalOutcome`, `notificationRedrive`, `scannerPresetScheduler`, `remoteConfigRefresh`) wait a random delay inside `[0, value]` ms — and the global default is also surfaced — so multi-worker deploys do not all hit Redis / Firestore / Firebase Remote Config at the same wall-clock second. Values reflect env / Remote Config / default precedence and are bounded to `[0, 30000]`.
