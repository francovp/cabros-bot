...
Machine-readable changelog derived from `changelog.json` at the repository root. The file is generated by `pnpm run build:changelog` (`scripts/generate-changelog.mjs`), which parses `git log` and groups commits by Conventional Commits prefix. The endpoint always returns 200 — an empty `entries` array plus a `reason` field (e.g. `changelog_not_generated`, `changelog_malformed`) signals that the file is missing or unreadable.

- `limit` is bounded to `1`-`50` and defaults to `20`.
- `type` accepts a comma-separated list of Conventional Commits prefixes (`feat`, `fix`, `perf`, `chore`, `docs`, `refactor`, `test`, `build`, `style`, `ci`, `revert`, `other`); invalid values return `400 INVALID_REQUEST`.
- `since` accepts an ISO-8601 timestamp or `YYYY-MM-DD` date.
- Each entry exposes `type`, `scope`, `summary`, `breaking`, `pr`, `author`, `mergedAt`, `commitSha`, `shortSha`, and `issues`. No secrets, API keys, or webhook URLs from commit messages are ever returned.
- The `summary` payload also includes `counts` (per-type totals in the window), `days` (per-day totals), `version`, `generatedAt`, `branch`, and `sinceTag`.

### GET /api/release-notes/:version

Returns a snapshot for a specific version identifier. The path parameter accepts either the current changelog's `version` field (e.g. `v0.1.0`) or an ISO-8601 date / `YYYY-MM-DD` string. Date versions include every entry merged on or after that date. Invalid identifiers return `404 NOT_FOUND`.

### Build-time changelog generation

Run `pnpm run build:changelog` before deploys to refresh `changelog.json`. The script accepts `--branch=<name>`, `--since=<iso|date>`, `--limit=<n>`, and `--tag=<tag>`. It is fail-open: git enumeration failures emit `{ entries: [], reason: "git_enumeration_failed", error: "<message>" }` rather than crash the build.
