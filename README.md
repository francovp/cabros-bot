...
- **Static-hosting security headers**: `firebase.json` declares `Content-Security-Policy` (with `frame-ancestors 'none'`, `default-src 'self'`, and the same Firebase/Auth/backend `connect-src` set as Helmet), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, and `Permissions-Policy` on both `**` and `/admin/**` so the hosted console is not embeddable in third-party frames. `src/admin/index.html` mirrors the same directives as `<meta http-equiv>` fallback, and `pnpm run build:hosting` regenerates the matching tags in `public/index.html` so the source of truth and the deployed assets stay in sync.
