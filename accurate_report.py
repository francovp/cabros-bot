...
def get_news_for_ticker(ticker):
    ...
    try:
        ...
        if titles:
            # Return first two titles separated by semicolon
...
def main():
    ...
    if overbought.empty:
        ...
        for _, row in overbought.iterrows():
            if not first:
                lines.append("")  # blank line between ticker blocks? Actually the example shows each ticker on its own line with a bullet, no blank lines. But we'll keep blank for readability.
            first = False
            ticker = row['Name']
            price = row['Price']
            change = row['Change %']
            rsi = row['Rsi9']
            # Brief note of caution
            caution = "RSI alto sugiere posible corrección"
            lines.append(f"- {ticker} - 0 - RSI {rsi:.1f} - {caution}")
    lines.append("")
    
    # Resumen
    lines.append("*=== RESUMEN ===")
    # Macro analysis - 2-3 sentences about market trend and tech/Chile/crypto sector
    lines.append("El mercado estadounidense muestra señalizaciones mixtas con cierta debilidad en acciones de crecimiento y relativa fortaleza en valor. El sector tecnológico presenta oportunidades en niveles de sobreventa, particularmente en empresas con fundamentos sólidos. Se mantiene cautela frente a posibles correcciones en el corto plazo.")
    lines.append("---")
