# feat: add tradingview alert report

## Summary

Adds a TradingView MCP-backed alert report endpoint that accepts complete `EXCHANGE:SYMBOL` identifiers, generates a grouped Spanish technical-analysis report, sends it through the existing notification channels, returns per-symbol plus delivery results, and caps analysis duration with a request-level deadline.

## Key Changes

### :chart_with_upwards_trend: TradingView report endpoint

- Added `POST /api/tradingview-alert` behind the existing API-key middleware.
- Parses request body `symbols` first, then falls back to `TRADINGVIEW_ALERT_SYMBOLS`.
- Returns `400 NO_SYMBOLS` when neither source provides symbols.
- Applies `TRADINGVIEW_ALERT_TIMEOUT_MS` as a total analysis deadline with a 60 second default and 120 second cap.
- Returns `504 TRADINGVIEW_ALERT_TIMEOUT` when the deadline expires before any symbol can be analyzed.
...
- Added request parsing and report formatting in `src/services/tradingview/tradingViewAlertReport.js`.
...
#### `src/controllers/webhooks/handlers/tradingViewAlert/tradingViewAlert.js`
...
#### `src/services/tradingview/tradingViewAlertReport.js`
...
docs/superpowers/
├── plans/
│   └── 2026-05-22-tradingview-alert-endpoint.md
└── specs/
    └── 2026-05-22-tradingview-alert-endpoint-design.md
src/controllers/webhooks/handlers/tradingViewAlert/
└── tradingViewAlert.js
src/services/tradingview/
└── tradingViewAlertReport.js
tests/integration/
└── tradingview-alert-endpoint.test.js
tests/unit/
├── retry-helper.test.js
├── tradingview-alert-report.test.js
...
- Request body symbols take precedence over `TRADINGVIEW_ALERT_SYMBOLS`.
...
- Endpoint deadline response with `504 TRADINGVIEW_ALERT_TIMEOUT`.
...
- `tests/unit/tradingview-alert-report.test.js`: parser and formatter coverage.
...
- `tests/integration/tradingview-alert-endpoint.test.js`: endpoint behavior, timeout handling, and notification dispatch coverage.
...
```bash
TRADINGVIEW_ALERT_SYMBOLS=BINANCE:BTCUSDT,NASDAQ:NVDA
TRADINGVIEW_ALERT_TIMEOUT_MS=60000
...
- [x] `pnpm test -- tests/unit/tradingview-alert-report.test.js tests/integration/tradingview-alert-endpoint.test.js --testTimeout=5000`
- [x] `pnpm test -- tests/unit/tradingview-mcp-service.test.js tests/unit/tradingview-alert-report.test.js tests/integration/tradingview-alert-endpoint.test.js --testTimeout=10000`
- [x] `pnpm test -- tests/unit/retry-helper.test.js tests/unit/tradingview-mcp-service.test.js tests/integration/tradingview-alert-endpoint.test.js --testTimeout=10000`
- [x] Live MCP smoke: 6-symbol request completed with status 200 in about 5 seconds.
- [x] Live timeout smoke: forced 1ms deadline returned `504 TRADINGVIEW_ALERT_TIMEOUT` in about 33ms.
- [x] `pnpm test`
- [x] `pnpm lint`

### Post-merge Verification

- [ ] Call `POST /api/tradingview-alert` with production notification env configured.
