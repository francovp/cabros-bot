...
- Updated `.agents/skills/trading-profit-opportunity-scout/SKILL.md`:
  - Added hard rule and step to fetch stored production Firebase alerts from the target window (e.g. last 2 weeks) using Render deployment endpoints (`GET /api/alerts/summary?from=<ISO>&to=<ISO>`, `GET /api/alerts/export?from=<ISO>&to=<ISO>&includeText=true`, `GET /api/alerts`) authenticated via `WEBHOOK_API_KEY`, explicitly providing bounded `from` and `to` ISO-8601 timestamps.
