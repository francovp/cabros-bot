...
- GH-173 / CB-69: `/api/status` and `/api/capabilities` expose `featureFlags.tradingViewVolumeConfirmation` plus `dependencies.tradingViewVolumeConfirmation` readiness without changing the existing `ENABLE_TRADINGVIEW_VOLUME_CONFIRMATION` gate.
