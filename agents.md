...
- GH-320 / CB-132: TradingView signal parsing and alert-storage extraction now preserve underscore-delimited exchange identifiers such as `FX_IDC:USDCLP(D)`; unknown exchange contexts stay neutral instead of inheriting the stock default, while known symbol, timeframe, auth, delivery, and fail-open behavior remain unchanged. Focused parser/storage regressions cover the recovered exchange, symbol, and neutral asset metadata.
