...
- `src/services/jobs/jobWorker.js` / `worker.js` — Claims queued jobs through Firestore transactions, releases claims only when BullMQ has another attempt, persists final worker failures, and drains the worker on `SIGTERM`.
