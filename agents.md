...
- GH-320 / CB-132: TradingView signal parsing and alert-storage extraction now preserve underscore-delimited exchange identifiers such as `FX_IDC:USDCLP(D)`; the known forex exchange stays neutral while unlisted equity exchanges retain the stock default, and known symbol, timeframe, auth, delivery, and fail-open behavior remain unchanged. Focused parser/storage regressions cover the recovered exchange, symbol, forex-neutral, and unlisted-equity metadata.
