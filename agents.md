...
- Durable worker saves reject stale writes that would replace a terminal Firestore job state, and ownerless nonterminal saves cannot replace an active claim; claim release and failure transitions also verify worker ownership and processing attempt transactionally, retrying terminal failure persistence while storage recovers.
...
- Render Blueprint worker wiring mirrors Telegram, WhatsApp, Discord, Firebase, TradingView, callback signing/validation overrides, and Sentry settings from the web service so queued deliveries, callback policy, and monitoring work in worker mode.
