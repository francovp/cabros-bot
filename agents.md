...
Status responses use a monotonic request version and pass an activity guard into `sendRequest`, so a response for a previously selected or manually replaced job cannot overwrite the current state, hold the shared form disabled, or render cancel/retry actions.
