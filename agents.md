...
- GH-320 / CB-132: TradingView signal parsing and alert-storage extraction now preserve underscore-delimited exchange identifiers such as `FX_IDC:USDCLP(D)`; existing symbol, timeframe, auth, delivery, and fail-open behavior are unchanged. Focused parser/storage regressions cover the recovered exchange and symbol metadata.
