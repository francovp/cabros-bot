...
- GH-279 / CB-111: signal-outcome evaluation now advances its in-process Firestore cursor only after each document is processed, so a max-duration interruption resumes at the unprocessed page tail instead of skipping it until cursor wraparound; regression coverage is in `tests/unit/signal-outcome-worker.test.js`.
