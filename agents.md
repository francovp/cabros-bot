...
- `src/services/jobs/jobWorker.js` / `worker.js` — Claims queued jobs through Firestore transactions, releases claims only when BullMQ has another attempt, persists final worker failures, and drains the worker on `SIGTERM` without stopping an unlaunched Telegraf transport.
...
- Render Blueprint worker wiring mirrors Telegram, WhatsApp, Discord, Firebase, TradingView, and `JOB_CALLBACK_SIGNING_SECRET` settings from the web service so queued deliveries and global callback signatures work in worker mode.
