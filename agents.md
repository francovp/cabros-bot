...
- GH-174 / CB-70: `/api/status` and `/api/capabilities` expose `featureFlags.firestoreJobStorage` plus `dependencies.firestoreJobStorage` readiness independently of alert storage, without changing the `ENABLE_FIRESTORE_JOB_STORAGE` gate.
