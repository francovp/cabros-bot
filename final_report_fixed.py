...
def main():
    ...
    if overbought.empty:
        ...
        for _, row in overbought.iterrows():
            if not first:
                lines.append("")  # blank line between ticker blocks? Actually we don't want blank lines between tickers in this section per format? The example shows each ticker on its own line, no blank lines. We'll not add blank lines.
            first = False
            ticker = row['Name']
            price = row['Price']
            change = row['Change %']
